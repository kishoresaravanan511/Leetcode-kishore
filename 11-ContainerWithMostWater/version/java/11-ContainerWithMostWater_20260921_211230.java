// Last updated: 9/21/2026, 9:12:30 PM
1// class Solution {
...
89
90
91        int n = height.length;
92        int left = 0;
93        int right = n-1;
94        int ans = 0;
95
96        while(left<=right)  //two pointer 
97        {
98            int hgt = Math.min(height[left],height[right]);
99            int width = right - left;
100
101            ans = Math.max(ans,hgt*width);
102
103            if(height[left] <= height[right])
104            {
105                left++;
106            }
107            else
